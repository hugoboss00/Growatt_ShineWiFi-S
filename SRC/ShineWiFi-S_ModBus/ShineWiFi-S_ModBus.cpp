...
void vSetAccumulatedEnergy()
{
  if (httpServer.hasArg("AcE"))
  {
    ...
    if (lAccumulatedEnergy <= 0)
    {
      ...
      sprintf(msg, "Setting accumulated value to %d", (int)httpServer.arg("AcE").toInt());
    }
    else
    {
...
void handlePostData()
{
  ...
  else
  {
    if (httpServer.arg("rd") == "Rd")
    {
      ...
      {
        sprintf(msg, "Read register %d with value %d", (int)httpServer.arg("reg").toInt(), u16Tmp);
      } 
      else 
      {
        sprintf(msg, "Read register %d impossible - not connected?", (int)httpServer.arg("reg").toInt());
      }
    }
    else
    {
      if (Inverter.WriteHoldingReg(httpServer.arg("reg").toInt(), httpServer.arg("val").toInt()))
        sprintf(msg, "Wrote Register %d to a value of %d!", (int)httpServer.arg("reg").toInt(), (int)httpServer.arg("val").toInt());
      else
        sprintf(msg, "Did not write Register %d to a value of %d - fault!", (int)httpServer.arg("reg").toInt(), (int)httpServer.arg("val").toInt());
    }
    httpServer.send(200, "text/plain", msg);
    return;
  }
